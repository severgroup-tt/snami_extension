  ...
  var r = function(t, n) {
    ...
    (r.count = (r.count || 0) + 1),
      ...
      (this._events = {
        input: {
          ...
          keydown: function(t) {
            ...
            s.opened &&
              (13 === e && s.selected
                ? (t.preventDefault(), s.select(void 0, void 0, t))
                : 9 === e && s.selected && s.tabSelect
                ? s.select(void 0, void 0, t)
                : 27 === e
                ? s.close({ reason: 'esc' })
                : (38 !== e && 40 !== e) ||
        ...
        ul: {
          ...
          click: function(t) {
            ...
            if (e !== this) {
              ...
              e && 0 === t.button && (t.preventDefault(), s.select(e, t.target, t));
            }
          },
        },
      }),
      ...
      this.input.hasAttribute('list')
        ? ((this.list = '#' + this.input.getAttribute('list')), this.input.removeAttribute('list'))
    ...
    goto: function(t) {
      ...
      this.selected && e[this.index].setAttribute('aria-selected', 'false'),
        ...
        t > -1 &&
          ...
          i.fire(this.input, 'awesomplete-highlight', { text: this.suggestions[this.index] }));
    },
    select: function(t, e, n) {
      if ((t ? (this.index = i.siblingIndex(t)) : (t = this.ul.children[this.index]), t)) {
        var s = this.suggestions[this.index];
        i.fire(this.input, 'awesomplete-select', { text: s, origin: e || t, originalEvent: n }) &&
          (this.replace(s),
          this.close({ reason: 'select' }),
          i.fire(this.input, 'awesomplete-selectcomplete', { text: s, originalEvent: n }));
      }
    },
    evaluate: function() {
      var e = this,
      ...
      i.length >= this.minChars && this._list && this._list.length > 0
        ? ((this.index = -1),
          ...
          0 === this.ul.children.length
            ? ((this.status.textContent = 'No results found'), this.close({ reason: 'nomatches' }))
            : (this.open(), (this.status.textContent = this.ul.children.length + ' results found')))
        : (this.close({ reason: 'nomatches' }), (this.status.textContent = 'No results found'));
    },
  }),
    (r.all = []),
    ...
    (r.ITEM = function(t, e, n) {
      return i.create('li', {
        innerHTML:
          '' === e.trim()
            ? t
            : t.replace(RegExp(i.regExpEscape(e.trim()), 'gi'), '<mark>$&</mark>'),
        role: 'option',
        'aria-selected': 'false',
